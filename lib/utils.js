...
function notify_server_when_transfer_succeeded(stdout, orig_cmd) {
...
function notify_server_when_transfer_failed(err_data, orig_cmd) {
  request.post(server_addr + ':' + cmd_port)
    .send(
      {'cmd': 'transfer_failed',
       'data' : err_data,
...
exports.transfer = function(cmd) {
  ...
  var child = exec(rsync_cmd, function (error, stdout, stderr) {
    if (error != null ) {
      ...
      notify_server_when_transfer_failed(error, cmd);

    } else if (stderr.length != 0) {
      console.error('exec error: ' + stderr);
      notify_server_when_transfer_failed(stderr, cmd);

    } else {
      notify_server_when_transfer_succeeded(stdout, cmd);
    }
  });
