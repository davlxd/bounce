...
function do_match_hosts = function() {
  if (set1.length === 0) {
...
function change_transferred_host_status(cmd_obj) {
...
function change_transfer_failed_host_status(cmd_obj) {
  var from_addr = cmd_obj.from;
  var to_addr = cmd_obj.to;
...
function provoke_trasfer(from_host, to_host) {
  console.log('Provoke trasferation, from ' + from_host.host + ':' + from_host.file_path + ', to ' + to_host.host + ':' + to_host.file_path);
  request.post(from_host.host + ':' + port)
    .send(
      {'cmd' : 'transfer',
       'from' : from_host.host,
	...
	return ;
      ...
      console.log('After matching hosts:');
      console.log('==================================');
      console.log(set0);
      console.log(set1);
      console.log('==================================');
